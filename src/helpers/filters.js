export default {
  ...
  orderByString(array, property, sortOrderProp, ignoreCase) {
    let sortOrder = sortOrderProp;
    if (sortOrder !== -1 && sortOrder !== 1) sortOrder = 1;
    return array.sort((a, b) => {
      let stringA = this.removeAccentsFromString(a[property]);
      let stringB = this.removeAccentsFromString(b[property]);
  ...
  matchStringFromQuery(query, stringProp) {
    let string = stringProp;
    const queryWords = query.toLowerCase().split(' ');
    string = string.toLowerCase();
    string = this.removeAccentsFromString(string);
    let match = true;
    queryWords.forEach((wordProp) => {
      let word = wordProp;
  ...
  removeAccentsFromString(strProp) {
    let str = strProp;
  ...
  getPersonFullName(person) {
    let fullName = person.name ? `${person.name}` : '';
